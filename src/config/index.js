...
if (env.NODE_ENV === 'development') {//开发环境
  config = {
    ...
    menuURL: 'http://localhost:8088/getResource',
    //存储和传递token的key和name
    tokenName: 'token',
    //弹框默认宽度
    dialogWidth: '800px',
...
cube.encryptData = function(p_data, url, method) {
  	//以下为一次一密及摘要示例，仅为参考。
  	//生成sm4的密钥
  let keypair = SG_sm2generateKeyPair()
  let	key4 = keypair.ecprvhex.substring(0, 16)
  ...
  // 对sm4的密钥使用sm2的公钥进行加密
  let auth1 = SG_sm2Encrypt(key4, config.key2)
  let auth2 = ''

  //对数据进行排序
  let	auth2Str = jsonSort(p_data, method) + '|' + key4
  //使用sm3生成摘要
  ...
  let customHeaders = cube.customHeaders
...
cube.decryptData = function(p_config, p_resultValue, textStatus, request, secretKey) {
  ...
  // 摘要对比
  let auth2 = SG_sm3encrypt(str)
  let responseAUTH2 = request.getResponseHeader('AUTH2')
