...
function RESTClient() {
  ...
  me.send = function(p_path, p_method, p_data, p_callback) {
    let url = me._makeUrl(p_path)

    let method = p_method !== null ? p_method.toUpperCase() : 'GET'

    let secretKey = null
    let customHeaders = me.customHeaders
    if (cube.encryptData) {
    ...
    // 发送axios请求
    let req = axios(ajaxSetting).then(function(response){
      let request = response.request
      let p_resultValue = response.data
      let textStatus = response.status

      let sessionstate = request.getResponseHeader('sessionstate')
  ...
  me.get = function(p_path, p_data, p_callback) {
    ...
    let req = me.send(p_path, 'GET', p_data, p_callback)
    return req
  }
  ...
  me.post = function(p_path, p_data, p_callback) {
    ...
    let req = me.send(p_path, 'POST', p_data, p_callback)
    return req
  }
  ...
  me._makeUrl = function(p_path) {
    ...
    let res = _getQueryString(result, 'authorization')
