...
function TreeEntityContainer() {
  let me = new entityContainer()
  let base = {}
  ...
  me.load = function(p_node, p_callBack) {
    ...
    if (typeof p_callBack !== 'undefined' && typeof p_node !== 'undefined') {
      _loadingNode = p_node
      let params = p_node.queryParams != null ? p_node.queryParams : {}
  ...
  me.getNodeEntity = function(p_nodeID, p_itemType) {
    if (
      cube.notEmpty(p_nodeID) &&
      (cube.isString(p_nodeID) || cube.isNumber(p_nodeID))
    ) {
      let node = null
      let datas = me.data
  ...
  me.create = function(p_node, p_options) {
    ...
    if (cube.isObject(p_options)) {
      args.parentNode = me.getNodeEntity(
        p_options.parentId,
        p_options.itemType
      )
    ...
    let event_args = Object.assign({ cancel: false }, args)
    ...
    let node = _create(args.node, args)
    args.node = node
    event_args = Object.assign({}, args)
    //	        me.trigger("created", event_args);
    return cube.isEmpty(node) ? node : Object.assign({}, node)
  }
  ...
  function _create(p_node, p_args) {
    ...
    if (data.indexOf(node) === -1) {
      ...
      if (cube.notEmpty(nextSibling)) {
        let index = data.indexOf(nextSibling)
        if (index !== -1) {
          data.splice(index, 0, node)
        }
      } else if (cube.notEmpty(prevSibling)) {
        let index = data.indexOf(prevSibling)
        if (index !== -1) {
    ...
    if (cube.isArray(node.childNodes)) {
      ...
      for (let i = 0; i < childNodes.length; i++) {
        _create(childNodes[i], { parentNode: node })
      }
    }
  ...
  function _filterNodes() {
    if (cube.notEmpty(_loadingNode)) {
      if (
        typeof _loadingNode.childNodes === 'function' &&
        cube.isArray(_loadingNode.childNodes())
      ) {
  ...
  function _clearChildren(p_node) {
    let children = cube.isEmpty(p_node) ? me.data : p_node.childNodes
  ...
  function _populateNodes(p_nodes, p_parentNode) {
    ...
    if (!cube.isArray(p_nodes)) return
    ...
    for (let i = 0; i < p_nodes.length; i++) {
      ...
      pk = node[me.primaryKey]
      node.parentId = cube.isEmpty(p_parentNode)
        ? null
        : p_parentNode[me.primaryKey]
      node.hasChildren = cube.obj(
        cube.isEmpty(node.hasChildren) ? false : node.hasChildren
      )
      node.expanded = cube.obj(
        cube.isEmpty(node.expanded) ? false : node.expanded
      )
      node.childNodes = cube.array(
        cube.isEmpty(node.childNodes) ? [] : node.childNodes
      )
      ...
      if (
        typeof node.childNodes === 'function' &&
        cube.isArray(node.childNodes())
      ) {
        let childNodes = node.childNodes
        _populateNodes(childNodes(), node)
      } else if (cube.isArray(node.childNodes)) {
        let childNodes = node.childNodes
        _populateNodes(childNodes, node)
      } else {
        node.childNodes = cube.array([])
  ...
  function _remove(p_node, p_parentNode) {
    let parent = p_parentNode

    if (
      typeof p_node.childNodes === 'function' &&
      cube.isArray(p_node.childNodes()) &&
      p_node.childNodes().length > 0
    ) {
      let childNodes = p_node.childNodes()
    ...
    if (cube.isEmpty(parent)) {
      let index = me.data.indexOf(p_node)
      if (index >= 0 && index < me.data.length) {
        me.data.splice(index, 1)
      }
    } else if (
      typeof parent.childNodes === 'function' &&
      cube.isArray(parent.childNodes())
    ) {
  ...
  function _delete_success(p_result) {
    if (cube.isArray(_deletingNode)) {
      for (let i = 0; i < _deletingNode.length; i++) {
        let removeNode = me.getNodeEntity(
          _deletingNode[i].id,
          _deletingNode[i].itemType
        )
        if (removeNode) {
...
export default TreeEntityContainer
