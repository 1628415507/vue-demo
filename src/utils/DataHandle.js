...
function DataHandle() {
  ...
  me.init = function(model, p_dataOption) {
    me.model = model//获取this指向，即me.model = this
    // 如果没有submit方法
    if (!me.model.submit) {
    ...
    me.searchData = model[dataOption.searchName]

    let obj = model[dataOption.name]
    me.fields = obj
  ...
  me.load = function() {
    ...
    if (args.cancel !== true) {
      if (dataOption.type === 'form') {
        ...
        me.loadGrid(dataOption, cube.clone(me.searchData))
      }
    }
  }
  ...
  me.loadGrid = function(dataOption, p_data) {
    ...
    let args = {metaParams:null, dataParams:null}
    ...
    let filterParam = _getFilterParam(p_data)
    if (cube.notEmpty(filterParam)) {
      let filter = { filter: filterParam}
      args.dataParams = Object.assign(filter, args.dataParams)
    }

    let page = {pageIndex:parseInt(me.pageIndex), pageSize:parseInt(me.pageSize)}
  ...
  me.search = function() {
    ...
    me.loadGrid(me.model.dataOption, cube.clone(me.searchData))
  }
  ...
  me.showDialog = function(p_url, p_params, p_width) {
    ...
    p_params.parent = me.model
		...
		import(/* webpackChunkName: "[request]" */ '@/views/' + p_url.substring(1)).then(function(component) {
