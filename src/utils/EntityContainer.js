...
function EntityContainer() {
  ...
  me.load = function() {
    ...
    let args = me._parseArgs(arguments)

    // 通过服务加载数据并且 ID 为空，是新增表单的场景。
    if (
      me.type === 'form' &&
      cube.isEmpty(args.path) &&
      cube.notEmpty(me.baseUrl)
    ) {
      args.path = 'null'
    }

    let p_path = args.path
    let p_data = args.para
    let p_callBack = args.callBack

    let loadingArgs = {
      cancel: false
    }
    loadingArgs = Object.assign(loadingArgs, me._setLoadingArgs())
    ...
    if (cube.notEmpty(me.baseUrl)) {
      if (cube.notEmpty(me.restClient)) {
        ...
        if (me._changingType && me.loadMeta) {
          ...
          let metaPath = me._makePath('meta', 'meta', null)
          if (cube.notEmpty(me.iscID) && me.iscID  !==  '-1') {
            metaPath =
              metaPath === null ? '/' + me.iscID : metaPath + '/' + me.iscID
          }
          // 根据 client 属性的 async 属性，设置调用方式是否同步。
          let _get = _getClientCallMode('meta')
        ...
        if (me._loadData) {
          let restPath = me._makePath('data', null, p_path)
          let _get = me.loadByPost
            ? me._getClientPostMode('data')
            : _getClientCallMode('data')
      ...
      // 加载本地数据。
      let localData = me.data
      let localMeta = me.meta
      me.data = null
      me.meta = null
      // 将本地数据信息模拟成与后台服务返回结果一样的格式。
      // 各数据容器的服务返回结果格式可能不同，因此模拟过程也可能不同。
      let localCall = function() {
        _load_success(
          me._prepareLocalData(localData, localMeta, dataPara),
          p_callBack
        )
      }
  ...
  me._makePath = function(p_action, p_defautAction, p_path) {
    let actionPath = p_action in me.actions ? me.actions[p_action] : null
  ...
  me.setValue = function(p_entity, p_col, p_value, p_triggerEvent) {
    ...
    // 获取要改变数据的主键。
    let dataPk = p_entity[me.primaryKey]
    ...
    let entityOldData = me._oldData[dataPk]
  ...
  me.save = function() {
    ...
    let savePara = JSON.stringify(_parseSavePara(p_data, p_saveAll))
    ...
    let restPath = me._makePath('save', 'save', p_path)
    if (cube.notEmpty(me.restClient)) {
      let _post = me._getClientPostMode('save')
  ...
  me.getChangedItems = function() {
    ...
    for (let i = 0; i < me._changedItems.length; i++) {
      items.push(me._changedItems[i])
    }
  ...
  me._parseData = function(p_result) {
    if (me.type === 'form') {
      if (cube.isEmpty(p_result) || cube.isEmpty(p_result.items)) {
        me.data = {}
      } else if (cube.isArray(p_result.items) && p_result.items.length > 0) {
      ...
      if (
        cube.isEmpty(me.data) ||
        (cube.isArray(me.data) && me.data.length === 0)
      ) {
        me.data = new Object()
      }
  ...
  me._parseDicts = function(p_result) {
    ...
    if (cube.isArray(me.dicts)) {
      for (let i = 0; i < me.dicts.length; i++) {
        let dict = me.dicts[i]
        if (
          cube.isObject(dict) &&
          typeof dict.name !== 'undefined' &&
          typeof dict.values !== 'undefined'
        ) {
          me.dicts[dict.name] = dict.values
          if (cube.isArray(dict.values)) {
            me.dictsKeyValue[dict.name] = {}
            for (let j = 0; j < dict.values.length; j++) {
              me.dictsKeyValue[dict.name][dict.values[j].value] =
                dict.values[j].text
            }
          }
        }
  ...
  me._resetNewData = function(p_result) {
    if (me.type === 'form') {
      // 将后台保存返回结果更新新增数据的主键。
      let saveResult = p_result.resultValue.items
      if (cube.isArray(saveResult) && saveResult.length > 0) {
        //后端可能返回整条数据，也可能是部分字段
        me.data = Object.assign(me.data, saveResult[0])
      }
      return saveResult
    } else {
      // 本方法主要用于保存完成后回写新增数据的主键，同时为 onsaved 事件准备参数。
      let resetArgs = new Object()
      resetArgs.deleteIDs = new Array()
      resetArgs.newData = new Array()
      // 删除带有伪主键的新增数据。
      for (let itemKey in me._changedItems) {
        let changedItem = me._changedItems[itemKey]
  ...
  me._setLoadArgs = function(p_result) {
    ...
    if (cube.notEmpty(p_result.page)) {
      let pageIndex = parseInt(p_result.page.pageIndex)
      let pageSize = parseInt(p_result.page.pageSize)
      let startRow = (pageIndex - 1) * pageSize
      let endRow = pageIndex * pageSize
  ...
  me._showError = function(p_msgKey, p_context) {
    let msg, errorPage
    if (cube.isObject(p_context)) {
      msg = p_context.resultHint || cube.msg('ERR_UNKNOWN')
      errorPage = p_context.errorPage
    } else {
      p_context = {
        type: 'error'
      }
    ...
    if (p_context.type !== 'error') {
      if (p_context.type === 'warn' || p_context.type === 'info') {
        if (me._viewModel) {
          cube.indicate.call(me._viewModel, p_context.type, msg)
        } else {
          cube.indicate(p_context.type, msg)
        }
      } else if (
        typeof p_context.error !== 'undefined' &&
        p_context.error === 'timeout'
      ) {
        if (me._viewModel) {
          cube.indicate.call(
            me._viewModel,
            'error',
            cube.msg('REQUEST_TIMEOUT')
          )
        } else {
        ...
        if (me._viewModel) {
          cube.indicate.call(me._viewModel, 'warning', msg)
        } else {
  ...
  me._notAllowedTip = function() {
    let msg = cube.msg('OPERATION_NOT_ALLOWED')
    let args = {
      message: msg,
      cancel: false
    }
  ...
  me.getEntityByKey = function(p_key) {
    let results = jsonUtil.getItems(me.data, me.primaryKey, p_key)
  ...
  me.remove = function(p_IDs, p_para, p_path) {
    ...
    me._deleteIDs = IDs
    if (cube.notEmpty(me.baseUrl)) {
      let delUrl = me._makePath('remove', 'delete', p_path)
      if (cube.notEmpty(me.restClient)) {
        ...
        if (delIDs.length === 0) {
          ...
          if (cube.notEmpty(p_para)) {
            delPara = Object.assign(delPara, p_para)
          }
          let _post = me._getClientPostMode('remove')
  ...
  me.create = function(p_entity, p_options) {
    ...
    let newPk = p_entity[me.primaryKey]
    let isNew = false
  ...
  function _load_data_callback(p_context, p_callBack) {
    if (cube.isEmpty(p_context)) return
  ...
  function _load_success(p_result, p_callBack) {
    ...
    me._loadData = true
    // 为 onload 事件准备参数。各数据控件的事件参数格式可能有差别。
    let args = me._setLoadArgs(p_result.resultValue, p_result.parentId)
    // 加载的数据不为空时，才可以触发load事件
    if (
      me.onload &&
      (typeof args.triggerLoad === 'undefined' || args.triggerLoad === true)
    ) {
      me.onload(args)
    }
  ...
  function _save_callback(p_context, p_callBack) {
    ...
    if (cube.isObject(p_context)) {
      ...
      if (p_context.successful) {
        ...
        if (
          (p_context.type === 'info' || p_context.type === 'warn') &&
          cube.isArray(p_context.resultValue)
        ) {
          //							me.trigger("validate", {
  ...
  function _save_success(p_result) {
    ...
    let saveResult = me._resetNewData(p_result)
    // 清除变更记录。
    me.clearChanges()
    let args = {
      p_result: p_result
  ...
  function _parseSavePara(p_data, p_saveAll) {
    ...
    if (!cube.isArray(p_data.items)) {
      ...
      if (cube.notEmpty(p_saveAll) && p_saveAll === true) {
        let saveData = cube.clone(me.data)
        if (cube.isArray(me.meta)) {
          for (let i = 0; i < me.meta.length; i++) {
      ...
      for (let i = 0; i < p_data.items.length; i++) {
        delete p_data.items[i].mxVirtualId
      }
    }
    // 模型驱动的情况下，将 clsID 添加到参数中
    if (cube.notEmpty(me.mdaParams)) {
  ...
  function _mergeResult(temp, newRes) {
    ...
    temp.resultValue = Object.assign(temp.resultValue, newRes.resultValue)
  }

  function _getClientCallMode(p_action) {
    let client = _getClient(p_action)
    return client.async === null || client.async === true
      ? client.get
      : client.getSync
  }
  ...
  function _getClient(p_action) {
    let client = me.restClient
    if (p_action in _clientMap) client = _clientMap[p_action]
    return client
  }
  ...
  function _delete_success(p_result) {
    for (let i = 0; i < me._deleteIDs.length; i++) {
      let deleteID = me._deleteIDs[i]
 ...
 * @ignore
 */
function JSON2Flat(meta, jsonUtil) {
  let me = {}
  ...
  me.setMeta = function(p_meta) {
    ...
    for (let i = 0; i < names.length; i++) {
      if (cube.notEmpty(names[i]) && names[i].indexOf('.')  !==  -1) {
  ...
  function _jsonFlat(p_item) {
    for (let i = 0; i < names.length; i++) {
      ...
      if (cube.isArray(name)) {
        ...
        let data = jsonUtil.jsonPath(p_item, queryString)
        if (data) {
    ...
    for (let key in p_item) {
      if (key.indexOf('.')  !==  -1) {
        ...
        for (let i = 0; i < length - 1; i++) {
          let name = names[i]
          if (cube.isEmpty(data[name])) data[name] = {}
          data = data[name]
        }
