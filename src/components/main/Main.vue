<template>
  <div>
    <el-tabs
      ...
      <el-tab-pane
        v-for="(item) in $store.state.tabs.editableTabs"
        :key="item.route"
        :label="item.name"
        :name="item.route">
        <component
  ...
  methods: {
	  ...
	  removeTab(targetName) {
        ...
        return
	      ...
	      let activeIndex = this.$store.state.tabs.activeIndex
	      let editableTabs = this.$store.state.tabs.editableTabs
	      let hasRoute = this.$store.state.tabs.hasRoute
