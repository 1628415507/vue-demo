...
var store = new Vuex.Store({
  ...
  mutations: {
	  ...
	  //添加选项卡
	  addTab (state, data) {
		  import(/* webpackChunkName: "[request]" */ '@/components/' + data.route.substring(1)).then(function(component) {
	  ...
	  deleteTab (state, route) {
	      var index = 0;
	      for (var option of state.tabs.editableTabs) {
...
var menusItem = localStorage.getItem('menus');
if (menusItem) {
	try {
