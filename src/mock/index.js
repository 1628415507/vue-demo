...
if ($config.mock) {
  Mock.mock(RegExp($config.baseUrl + '/user/login'),  (options) =>{
    return Mock.mock({'successful':true,'resultValue':{'userName':'张三','token':'111111'},'resultHint':'','errorPage':'','type':''})
  })
  ...
  Mock.mock(RegExp($config.baseUrl + '/department/.*'),  (options) =>{
    ...
    let lastUrl = getLastUrl(options.url)
    switch (lastUrl) {
    case '': //列表
      result.resultValue = {
        'itemCount': 8,
